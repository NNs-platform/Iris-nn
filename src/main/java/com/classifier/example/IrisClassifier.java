package com.classifier.example;
...
public class IrisClassifier {
	...
	public void classify(String irisDataTrainFile, String irisDataTestFile)
		...
		Map<Integer, Iris> irises = objectify(testData);
        System.out.println("TestData");
        System.out.println(testData.toString());
        System.out.println("testData...." + testData.getFeatures().toString());
		irises.forEach((k, v) -> System.out.println("Index:" + k + " -> " + v));
	...
	private Map<Integer, Iris> objectify(DataSet testData) {
		Map<Integer, Iris> irises = new HashMap<>();
		INDArray features = testData.getFeatureMatrix();
		for (int i = 0; i < features.rows(); i++) {
			INDArray slice = features.slice(i);
			System.out.println("slicing" + slice.toString());
			Iris iris = new Iris(slice.getDouble(0), slice.getDouble(1), slice.getDouble(2), slice.getDouble(3));
			irises.put(i, iris);
		}
		return irises;
	}

	private void classify(INDArray output, Map<Integer, Iris> irises) {
		for (int i = 0; i < output.rows(); i++) {
			Iris irs = irises.get(i);
